//! Arbiter queue for human-in-the-loop action approval.
//!
//! AI agents submit actions via MCP; each gets a UUID and enters the pending
//! queue. A human approves or denies in the TUI arbiter tab before execution.
...
pub struct PendingAction {
    ...
    /// Source agent identifier (e.g., "claude-agent").
    pub source: String,
}

/// Queue of agent actions awaiting human approval.
///
/// Designed to be shared as `Arc<Mutex<ArbiterQueue>>` between the MCP
/// server (writer) and TUI arbiter tab (reader/approver).
